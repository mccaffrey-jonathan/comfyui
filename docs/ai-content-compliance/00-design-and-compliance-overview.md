...
| Corpus run: the detector's fine-grid ring guard rejected every ring above 2x upscale, so the advertised 0.4-2.5x scale search stopped at 2.0x and a 400 px image resized to 1080 px was never detected | Guard now scales with the hypothesis (half the expected steps, at least one); regression test at 2.6x; default search widened to 0.3-3x in library, node and CLI (about 25 % more detection time) after measuring that 0.35x downscales go from 0/12 to 9/12 detections |
