...
| Corpus run: the detector's fine-grid ring guard rejected every ring above 2x upscale, so the advertised 0.4-2.5x scale search stopped at 2.0x and a 400 px image resized to 1080 px was never detected | Guard now scales with the hypothesis (half the expected steps, at least one); regression test at 2.6x; default search widened to 0.4-3x in library, node and CLI (about 8 % more detection time); a 0.3x floor was measured too (0.35x downscales go from 0/12 to 9/12 detections) but left opt-in because the wider null search costs 3-4 points of clean-channel payload recovery |
| Corpus run: the adaptive strength policy raised strength on exactly the images where the mark was visible (canny edge maps, high-contrast line art), because it used in-band energy but not the flat fraction it already computed | Texture boost is now gated on flat fraction below 0.3; the size rules are unchanged |
